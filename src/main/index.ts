...
import APISchemaService from './apiSchemaService/main'
...
app.whenReady().then(() => {
  ...
  ipcMain.handle('loadYaml', async (_event, path: string) => {
    console.log(path)
    return APISchemaService.openAPISchemaOfPath(path)
  })

  ipcMain.handle('saveYaml', async (_event, path: string, data: object) => {
  ...
  ipcMain.handle(
    'createYaml',
    async (_event, path: string, fileName: string, data: object): Promise<string> => {
